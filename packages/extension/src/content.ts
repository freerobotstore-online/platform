...
function showIndicator() {
  ...
  indicator.setAttribute('data-frs-ignore', '');
  ...
  indicator.innerHTML =
    '<span style="width:8px;height:8px;border-radius:50%;background:white;animation:frs-pulse 1s infinite"></span> Recording';
  const style = document.createElement('style');
  style.textContent = '@keyframes frs-pulse { 0%,100% { opacity:1 } 50% { opacity:0.3 } }';
...
chrome.runtime.onMessage.addListener((msg: ExtMessage, _sender, sendResponse) => {
  switch (msg.type) {
    ...
    case 'CAPTURE_HTML': {
      ...
      clone
        .querySelectorAll('script, style, link, noscript, svg, [data-frs-ignore]')
        .forEach((n) => n.remove());
