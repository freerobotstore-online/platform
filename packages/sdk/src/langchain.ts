/**
 * LangChain.js integration for FreeRobotStore agents.
 * Wraps LangChain with FRS proxy for API key injection.
 *
 * Usage:
 *   import { createFagsChat, createFagsEmbeddings } from '@freerobotstore/sdk/langchain'
...
/**
 * Create a chat model that routes through the FRS proxy.
...
export function createFagsChat(options: FagsChatOptions) {
  ...
  const origin =
    typeof window !== 'undefined' ? window.location.origin : 'https://freerobotstore.online';
  const proxyBase = `${origin}/v1/proxy/${host}`;

  return {
    async invoke(messages: Array<{ role: string; content: string }>): Promise<string> {
      const token = getToken();
      if (!token) throw new Error('Not signed in. Call frs.keys.manage() first.');
...
/**
 * Configuration for using LangChain.js with FRS proxy.
 * Returns the config object to pass to LangChain's ChatOpenAI constructor.
 *
 * Usage with LangChain:
 *   import { ChatOpenAI } from '@langchain/openai'
 *   import { getFagsLangChainConfig } from '@freerobotstore/sdk/langchain'
...
export function getFagsLangChainConfig(model: string, provider = 'openai') {
  ...
  return {
    ...
    temperature: 0.7,
    configuration: {
      baseURL: `${typeof window !== 'undefined' ? window.location.origin : 'https://freerobotstore.online'}/v1/proxy/${host}/v1`,
      defaultHeaders: { Authorization: `Bearer ${token}` },
    },
    // LangChain needs an API key param even though proxy injects it
    openAIApiKey: 'frs-proxy-managed',
  };
}
