...
const ORG = 'FreeRobotStore';
const DOMAIN = 'freerobotstore.online';
...
export async function publish(opts: { name?: string; category?: string }) {
  ...
  process.stdout.write(`Publishing ${agentId} to FreeRobotStore...\n\n`);
  ...
  try {
    ...
    execFileSync('npx', ['wrangler', 'd1', 'execute', 'frs', '--remote', '--command', sql], {
      stdio: 'pipe',
    });
