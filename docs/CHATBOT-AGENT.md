...
Uses the FRS key vault + proxy:
1. User stores their OpenAI/Anthropic/Google key once at `freerobotstore.online/console/` → Profile → API Keys
...
Future: chunking + embeddings via `@freerobotstore/smart-search` agent for better retrieval.
...
### Proxy call pattern
```typescript
const response = await fetch('https://freerobotstore.online/v1/proxy/api.openai.com/v1/chat/completions', {
  method: 'POST',
  headers: {
...
frs publish      # deploys to freerobotstore.online/a/chatbot/
```
