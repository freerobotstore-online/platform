...
platform/
├── packages/
│   ├── sdk/            @freerobotstore/sdk — model loading, inference, Ollama, heuristics
│   ├── cli/            @freerobotstore/cli — `frs` binary (init, check, publish)
│   ├── compliance/     Agent-specific compliance checks (9 checks)
│   ├── backend/        API Worker (placeholder)
│   └── quality/        Quality reporter (placeholder)
├── workers/
│   ├── host/           *.freerobotstore.online → D1 → R2 host worker
│   └── mcp/            MCP server (11 tools) at mcp.freerobotstore.online
├── agents/
...
import { useModel, useWorkerInference, useOllama } from '@freerobotstore/sdk/hooks';
...
import { evaluateHeuristic, buildEvolvePrompt } from '@freerobotstore/sdk';
```

## MCP tools

Connect via `npx mcp-remote https://mcp.freerobotstore.online/mcp`
...
| Resource | Name/ID |
|---|---|
| CF Zone | `freerobotstore.online` |
| D1 | `frs` |
| R2 | `frs-robots` |
| KV (OAuth) | `FRS_OAUTH_KV` |
| Doppler | project `frs` |
| GitHub Org | `FreeRobotStore` |
...
| **FreeRobotStore** | **Browser AI tools** | **https://freerobotstore.online** |
