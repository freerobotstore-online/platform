...
export class FagsMcpAgent extends McpAgent<Env, unknown, McpProps> {
  ...
  async init() {
    ...
    this.server.tool(
      'list_agents',
      'List all published agents on FreeRobotStore, or just your own if authenticated.',
      ...
      async ({ mine }) => {
        if (mine) {
          ...
          const data = (await frsApi(this.env.API_BASE, '/v1/apps/mine', token)) as {
            apps?: Array<{
              id: string;
              category: string;
              oneliner: string;
        ...
        const lines = rows.results.map(
          (r) => `- **${r.slug}** → https://freerobotstore.online/a/${r.slug}/`,
        );
    ...
    this.server.tool(
      ...
      async ({ agent_id }) => {
        ...
        const liveUrl = `https://freerobotstore.online/a/${agent_id}/`;
    ...
    this.server.tool(
      'create_agent',
      'Create a new agent on FreeRobotStore — provisions GitHub repo, R2 route, and DNS. Requires auth + GITHUB_TOKEN.',
      {
        agent_id: z
          .string()
          .regex(/^[a-z0-9-]+$/)
      ...
      async ({ agent_id, name, description, template }) => {
        ...
        const existing = await this.env.DB.prepare(
          "SELECT slug FROM routes WHERE slug = ? AND zone = 'freerobotstore.online'",
        )
          .bind(agent_id)
          .first();
        if (existing)
          return text(
            `Agent **${agent_id}** already exists at https://freerobotstore.online/a/${agent_id}/`,
          );
        ...
        await this.env.DB.prepare(
          "INSERT INTO routes (slug, zone, r2_prefix, store, hosted_on, created_at, updated_at) VALUES (?, 'freerobotstore.online', ?, 'agents', 'r2', strftime('%s','now'), strftime('%s','now'))",
        )
        ...
        return text(
          [
            ...
            `URL: https://freerobotstore.online/a/${agent_id}/ (live after first deploy)`,
            ``,
            `Next steps:`,
            `1. Clone: \`git clone https://github.com/${org}/${agent_id}\``,
            `2. Scaffold from template: \`frs init ${agent_id}${template ? ` --template ${template}` : ''}\``,
    ...
    this.server.tool(
      ...
      async ({ agent_id, archive_repo }) => {
        ...
        const result = await this.env.DB.prepare(
          "DELETE FROM routes WHERE slug = ? AND zone = 'freerobotstore.online'",
        )
          .bind(agent_id)
          .run();
    ...
    this.server.tool(
      'platform_guide',
      'Get the FreeRobotStore platform guide — architecture, SDK reference, and how to build browser-based AI agents.',
      {},
      async () => {
        // Return a condensed guide
        return text(
          [
            '# FreeRobotStore Platform Guide',
            '',
            '## What it is',
            ...
            '',
            '## SDK: @freerobotstore/sdk',
            '```tsx',
            "import { initAgent } from '@freerobotstore/sdk'",
            "const agent = initAgent({ agentId: 'my-agent' })",
            '',
            '// Hooks for React:',
            "import { useModel, useWorkerInference, useOllama, useResultStore } from '@freerobotstore/sdk/hooks'",
            '```',
            '',
            '## Templates',
            ...
            '',
            '## Deploy',
            'Push to main → GitHub Actions → R2 → live at freerobotstore.online/a/{agent}/',
            '',
            '## Compliance',
    ...
    this.server.tool(
      'sdk_reference',
      'Quick reference for @freerobotstore/sdk — model loading, inference, Ollama, storage.',
      {
        feature: z
      ...
      async ({ feature }) => {
        const sections: Record<string, string> = {
          model: `## Model Loading
\`\`\`tsx
import { useModel } from '@freerobotstore/sdk/hooks'
...
\`\`\`tsx
import { useWorkerInference } from '@freerobotstore/sdk/hooks'
...
\`\`\`tsx
import { useOllama } from '@freerobotstore/sdk/hooks'
...
Ollama at localhost:11434. User sets OLLAMA_ORIGINS="*" for CORS.`,
          cache: `## Model Cache
\`\`\`tsx
import { useModelCache } from '@freerobotstore/sdk/hooks'
...
\`\`\`tsx
import { useResultStore } from '@freerobotstore/sdk/hooks'
...
\`\`\``,
        ...
        return text(`# @freerobotstore/sdk Reference\n\n${selected}`);
      },
    );
  }
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === '/' || url.pathname === '') {
      return new Response(
        [
          'FreeRobotStore MCP Server',
          '',
          'Connect: npx mcp-remote https://mcp.freerobotstore.online/mcp',
          '',
          'Tools:',
