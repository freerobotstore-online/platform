/**
 * FRS Host Worker — API routes for auth, user key vault + proxy.
 *
 * Routes:
...
async function verifySession(request: Request, env: Env): Promise<string | null> {
  ...
  if (!token) {
    ...
    const match = cookie.match(/(?:^|;\s*)frs_session=([^\s;]+)/);
...
export async function handleApiRoute(request: Request, url: URL, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (path === '/v1/auth/github' && request.method === 'GET') {
      ...
      h.append(
        'Set-Cookie',
        `frs_oauth_state=${state}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=600`,
      );
      h.append(
        'Set-Cookie',
        `frs_return_to=${encodeURIComponent(returnTo)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=600`,
      );
    ...
    if (path === '/v1/auth/callback' && request.method === 'GET') {
      ...
      const stateMatch = cookies.match(/(?:^|;\s*)frs_oauth_state=([^\s;]+)/);
      ...
      const userRes = await fetch('https://api.github.com/user', {
        headers: {
          ...
          'User-Agent': 'FreeRobotStore-Host',
      ...
      const returnMatch = cookies.match(/(?:^|;\s*)frs_return_to=([^\s;]+)/);
      ...
      h.append(
        'Set-Cookie',
        `frs_session=${sessionToken}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=2592000`,
      );
      // Clear the state + return_to cookies
      h.append(
        'Set-Cookie',
        'frs_oauth_state=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0',
      );
      h.append('Set-Cookie', 'frs_return_to=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0');
      return new Response(null, { status: 302, headers: h });
    }
    ...
    if (path === '/v1/auth/logout' && request.method === 'POST') {
      const h = corsHeaders();
      h.set('Content-Type', 'application/json; charset=utf-8');
      h.append('Set-Cookie', 'frs_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0');
