 ...
 *
 * Injects a <script src="/v1/mirror.js"> tag and a <frs-mirror> Web Component
...
export const MIRROR_CLIENT_JS = `(function() {
  ...
  class FagsMirrorElement extends HTMLElement {
    ...
    /** Public API: agents call window.frsMirror.send({ type: 'result', data: {...} }) */
    send(data) {
  ...
  if (!customElements.get("frs-mirror")) {
    customElements.define("frs-mirror", FagsMirrorElement);
  }

  // Expose for programmatic use: window.frsMirror = document.querySelector('frs-mirror')
  Object.defineProperty(window, "FagsMirror", { value: FagsMirrorElement, writable: false });

  // Convenience: expose the first instance as window.frsMirror once it connects
  var _origDefine = customElements.define;
  requestAnimationFrame(function check() {
    var el = document.querySelector("frs-mirror");
    if (el) { window.frsMirror = el; }
