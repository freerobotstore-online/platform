...
function makeRequest(
  ...
  return new Request(`https://freerobotstore.online${path}`, {
    method,
...
describe('handleApiRoute', () => {
  ...
  it('GET /v1/keys/providers returns 6 providers', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/keys/providers'),
      mockEnv(),
    );
  ...
  it('GET /v1/keys/status without auth returns 401', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/keys/status'),
      mockEnv(),
    );
  ...
  it('PUT /v1/keys/openai without auth returns 401', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/keys/openai'),
      mockEnv(),
    );
    expect(res.status).toBe(401);
  });
  ...
  it('GET /v1/auth/me with valid token returns user info', async () => {
    ...
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/auth/me'),
      mockEnv({ SESSION_SIGNING_KEY: signingKey }),
    );
    expect(res.status).toBe(200);
  ...
  it('unknown /v1/ route returns 404', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/nonexistent'),
      mockEnv(),
    );
  ...
  it('GET /v1/auth/github redirects to GitHub OAuth', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/auth/github'),
  ...
  it('POST /v1/auth/logout clears session cookie', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/auth/logout'),
      mockEnv(),
    );
    expect(res.status).toBe(200);
    const cookie = res.headers.get('Set-Cookie') ?? '';
    expect(cookie).toContain('frs_session=');
  ...
  it('GET /v1/usage without auth returns 401', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/usage'),
      mockEnv(),
    );
    expect(res.status).toBe(401);
  });
  ...
  it('GET /v1/keys redirects to console', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/keys'),
      mockEnv(),
    );
    expect(res.status).toBe(302);
  ...
  it('auth via cookie works', async () => {
    ...
    const res = await handleApiRoute(
      makeRequest('GET', '/v1/auth/me', { headers: { Cookie: `frs_session=${token}` } }),
      new URL('https://freerobotstore.online/v1/auth/me'),
      mockEnv({ SESSION_SIGNING_KEY: signingKey }),
    );
    expect(res.status).toBe(200);
  ...
  it('PUT /v1/keys/unknown-provider returns 400', async () => {
    ...
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/keys/nonexistent'),
      mockEnv({ SESSION_SIGNING_KEY: signingKey }),
    );
  ...
  it('GET /v1/qr without data param returns 400', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/qr'),
      mockEnv(),
    );
    expect(res.status).toBe(400);
    const body = (await res.json()) as { error: string };
  ...
  it('GET /v1/qr with data returns SVG', async () => {
    const res = await handleApiRoute(
      makeRequest('GET', '/v1/qr'),
      new URL('https://freerobotstore.online/v1/qr?data=https://example.com'),
      mockEnv(),
    );
    expect(res.status).toBe(200);
  ...
  it('GET /v1/mirror.js returns JavaScript', async () => {
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/mirror.js'),
      mockEnv(),
    );
    expect(res.status).toBe(200);
  ...
  it('GET /v1/mirror/:roomId returns room info', async () => {
    ...
    const res = await handleApiRoute(
      ...
      new URL('https://freerobotstore.online/v1/mirror/abcdef12'),
  ...
  it('GET /v1/auth/github sets state cookie', async () => {
    ...
    expect(setCookie).toContain('frs_oauth_state=');
  ...
  it('GET /v1/auth/github with return_to sets cookie', async () => {
    const res = await handleApiRoute(
      makeRequest('GET', '/v1/auth/github'),
      new URL('https://freerobotstore.online/v1/auth/github?return_to=/console/'),
      mockEnv({ GITHUB_CLIENT_ID: 'test-id' } as unknown as Env),
    );
    const setCookie = res.headers.get('Set-Cookie') ?? '';
    expect(setCookie).toContain('frs_return_to=');
