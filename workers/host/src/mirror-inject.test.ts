...
describe('injectMirror', () => {
  it('injects script and web component before </body>', () => {
    ...
    expect(result).toContain('<frs-mirror agent="my-agent"></frs-mirror>');
  ...
  it('preserves content before and after </body>', () => {
    ...
    expect(result.indexOf('<frs-mirror')).toBeLessThan(result.indexOf('</body>'));
  });
});
...
describe('MIRROR_CLIENT_JS', () => {
  ...
  it('registers the custom element', () => {
    expect(MIRROR_CLIENT_JS).toContain('customElements.define("frs-mirror"');
  });
