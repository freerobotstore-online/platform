...
const GITHUB_ORG = 'FreeRobotStore';
const SESSION_KEY = 'frs:session';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/repos/${GITHUB_ORG}/platform/actions/runs?per_page=5`, {
      headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'frs-console' },
    })
      .then(r => r.json())
      .then(data => {
  ...
  const signIn = () => {
    ...
    window.location.href = `${AUTH_API}/v1/auth/github/start?app_id=frs-console&response_mode=query&return_to=${encodeURIComponent(returnTo)}`;
  };
    ...
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center ${isDark ? 'bg-neutral-950' : 'bg-gradient-to-br from-gray-50 via-white to-violet-50'}`}>
        ...
        <p className={`mb-6 ${isDark ? 'text-neutral-400' : 'text-gray-500'}`}>
          Sign in with GitHub to manage your AI agents on FreeRobotStore.
        </p>
        <button
          onClick={signIn}
        ...
        <p className={`mt-4 text-sm ${isDark ? 'text-neutral-500' : 'text-gray-400'}`}>
          Part of <a href="https://freerobotstore.online" className="underline">FreeRobotStore</a>
        </p>
      </div>
    );
  }
      ...
      <header className={`border-b ${isDark ? 'border-neutral-800 bg-neutral-950' : 'border-gray-200 bg-white'} sticky top-0 z-10`}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <nav className="flex gap-4 ml-auto text-sm font-medium">
            <a href="https://freerobotstore.online" className={isDark ? 'text-neutral-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}>Store</a>
            <a href="https://freerobotstore.online/skills.md" className={isDark ? 'text-neutral-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}>Docs</a>
          ...
          <>
            ...
            {agents.length === 0 && (
              <div className={`text-center py-12 ${isDark ? 'text-neutral-500' : 'text-gray-400'}`}>
                ...
                <p className="text-sm">Create one with <code className={`px-1 py-0.5 rounded text-xs ${isDark ? 'bg-neutral-800' : 'bg-gray-100'}`}>frs init my-agent</code> or via the MCP server.</p>
              </div>
            )}
          </>
        )}
    ...
    <div>
      ...
      <div className={`rounded-xl border p-6 mb-4 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-gray-200'}`}>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          <a href={`https://freerobotstore.online/agents/${agent.id}/`}
             className={`text-xs px-3 py-1.5 rounded-lg border font-medium no-underline ${isDark ? 'border-neutral-700 text-neutral-300' : 'border-gray-300 text-gray-700'}`}>
            Detail Page
          </a>
