...
function generateDetailPage(agent) {
  ...
  const repoPath = `FreeRobotStore/${agent.id}`;
...
<head>
  ...
  <title>${agent.name} — FreeRobotStore</title>
  <meta name="description" content="${agent.description} Free, private, runs in your browser.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="${agent.name} — FreeRobotStore">
  <meta property="og:description" content="${agent.description}">
  <meta property="og:url" content="https://freerobotstore.online/agents/${agent.id}/">
  <link rel="canonical" href="https://freerobotstore.online/agents/${agent.id}/">
  <script type="application/ld+json">${JSON.stringify({
    ...
    isPartOf: { '@id': 'https://freerobotstore.online/#website' },
  })}</script>
...
<body>
  <header>
    <div class="container">
      <a href="https://freerobotstore.online" class="brand">
      ...
      <nav>
        <a href="https://freerobotstore.online">Agents</a>
        <a href="https://freerobotstore.online/skills.md">Docs</a>
        <a href="https://console.freerobotstore.online">Console</a>
        <a href="https://github.com/FreeRobotStore">GitHub</a>
  ...
  <main class="container">
    <a href="https://freerobotstore.online" class="back">&larr; All agents</a>
    ...
    <div class="detail-split">
      <div>
        ...
        <div class="actions">
          ...
          <a href="https://console.freerobotstore.online" class="btn-secondary">Console</a>
        </div>
        ...
        <div class="section">
          ...
          <div style="background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:0.75rem;font-family:monospace;font-size:0.78rem;overflow-x:auto;color:var(--muted-soft)">
            ...
            <div style="color:var(--muted)">pnpm add ${agent.npmPkg ?? '@freerobotstore/' + agent.id}</div>
            ${agent.noEsm ? '' : `<div style="margin-top:0.5rem"># or import directly (zero install)</div>
            <div style="color:var(--muted)">import { ... } from '${agent.esmUrl ?? 'https://freerobotstore.online/pkg/' + agent.id + '/index.js'}'</div>`}
          </div>`
            : agent.api?.note
          ...
          <div style="background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:1rem;font-family:monospace;font-size:0.82rem;overflow-x:auto">
            ...
            <div>pnpm add ${agent.npmPkg ?? '@freerobotstore/' + agent.id}</div>
            <div style="color:var(--muted-soft);margin-top:0.75rem"># or import directly (zero install)</div>
            <div style="color:#a78bfa">import { ... } from '${agent.esmUrl ?? 'https://freerobotstore.online/pkg/' + agent.id + '/index.js'}'</div>
          </div>`
          }
        </div>
        ...
        <div class="section">
          ...
          <p style="color:var(--muted)">No apps yet. <a href="https://freeappstore.online">Build one on FreeAppStore</a> and import <code style="background:var(--panel);padding:0.1rem 0.4rem;border-radius:4px;font-size:0.82rem">${agent.npmPkg ?? '@freerobotstore/' + agent.id}</code>.</p>`}
        </div>
  ...
  <footer>
    <div class="container">
      <a href="https://freerobotstore.online">FreeRobotStore</a> &middot;
      <a href="https://freerobotstore.online/skills.md">Docs</a> &middot;
      <a href="https://github.com/FreeRobotStore">GitHub</a> &middot;
  ...
  <script>
    (async function() {
      try {
        const r = await fetch('https://api.github.com/repos/FreeRobotStore/${agent.id}/readme', {
  ...
  <script>
    (function() {
      if (location.search.includes('login=success') && location.hash.startsWith('#session=')) {
        ...
        if (token) {
          localStorage.setItem('frs_session', JSON.stringify({ token: token }));
      ...
      var session = null;
      try {
        var stored = localStorage.getItem('frs_session');
      ...
      if (session && session.token) {
        try {
          ...
          if (payload.exp && payload.exp < Date.now() / 1000) {
            localStorage.removeItem('frs_session');
            session = null;
          } else {
...
function generateSandbox(agent) {
  ...
  const esmUrl = agent.esmUrl ?? `https://freerobotstore.online/pkg/${agent.id}/index.js`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${developer} — FreeRobotStore Developer</title>
  <meta name="description" content="${developer} has ${agents.length} agent${agents.length !== 1 ? 's' : ''} on FreeRobotStore.">
  <link rel="canonical" href="https://freerobotstore.online/developers/${slug}/">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,700&family=Manrope:wght@400;500;600;700&display=swap" rel="stylesheet">
  <style>
...
<body>
  ...
  <main class="container" style="padding-top:2rem;padding-bottom:2rem">
    <a href="https://freerobotstore.online" style="font-size:0.88rem;color:var(--muted);text-decoration:none">&larr; All agents</a>
