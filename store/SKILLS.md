...
```
See https://freerobotstore.online/skills.md for platform skills.
```
...
FreeRobotStore hosts browser-based AI tools in 3 types:
...
All agents are MIT-licensed, open source, one repo per agent in the `FreeRobotStore` GitHub org.
...
```bash
npx @freerobotstore/cli init my-agent --template heuristic
...
├── agent.json       — manifest
├── package.json     — @freerobotstore/my-agent
└── tsconfig.json
```
...
```bash
npx @freerobotstore/cli check    # compliance scan
npx @freerobotstore/cli publish  # creates repo, deploys, registers route
```
...
- **Header**: FreeRobotStore link + agent name + type badge
...
```typescript
fetch('https://freerobotstore.online/v1/proxy/api.openai.com/v1/chat/completions', {
  method: 'POST',
...
```bash
npx mcp-remote https://mcp.freerobotstore.online/mcp
```
