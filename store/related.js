// Related Simulations — FreeRobotStore
// Include at bottom of any demo page: <script src="/related.js"></script>
(function() {
  const ACCENT = '#f97316';
  const ITEM_PATH = '/robots/';
  const REGISTRY_KEY = 'robots';
  const STORE_NAME = 'FreeRobotStore';

  function getCurrentId() {
    const path = window.location.pathname;
    const match = path.match(/\/robots\/([^/]+)/);
  ...
  function render(current, items) {
    ...
    container.innerHTML = '<style>' +
      ...
      '.rel-toggle.show{bottom:.4rem;opacity:1}' +
      '</style>' +
